...
var partial_intra_L_q{t in TEAMS, i in FLEET[t], j in FLEET[t]: i != j} =
   (-1)/(sigmasqr/varrho + sum{k in ALL_AGENTS: k not in FLEET[t]} q_J[i, k]) + lambda[i] * dist[i, j]^alpha - nu[i, j];

var partial_inter_L_q{t in TEAMS, i in FLEET[t], l in ALL_AGENTS: l not in FLEET[t]} = 
   sum{t2 in TEAMS, k in FLEET[t2]: t2 != t and k != l} (- q[t2, k, l])/(sigmasqr/varrho + sum{j in FLEET[t]} q_J[j, l]) + lambda[i] * dist[i, l]^alpha - nu[i, l];
...
var partial_L_x{t in TEAMS, i in FLEET[t]} = 
   ...
   + mu_dist[i] * (x[i] - x0[i])/(sqrt((x[i] - x0[i])^2 + (y[i] - y0[i])^2));
...
KKT_power_stationary_2{t in TEAMS, i in FLEET[t], k in ALL_AGENTS: k not in FLEET[t]}:
   partial_inter_L_q[t, i, k] == 0;
